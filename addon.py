...
# Python 2.7's SSL is too old for modern PyPI; disable auto-upgrade by default.
# Set to True explicitly only if your Python 3+ engine can reach PyPI.
AUTO_UPGRADE = False
...
def _invalidate_import_caches():
    """Invalidate import caches (Python 3.3+), noop on older interpreters."""
    try:
...
def _import_bridge():
    ...
    # Delete ALL itasca_mcp_bridge submodules from cache so Python 2.7's
    # relative-import machinery doesn't trip over stale module objects.
    for _key in list(sys.modules):
        if _key == "itasca_mcp_bridge" or _key.startswith("itasca_mcp_bridge."):
...
def main():
    ...
    # Explicitly pass auto_upgrade=False — the env var is unreliable across
    # bridge versions (some read it before the import guard, causing crash).
    itasca_mcp_bridge.start(port=PORT, auto_upgrade=False)
