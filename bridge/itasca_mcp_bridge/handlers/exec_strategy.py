...
from .. import _compat as futures_compat
import logging
...
def _terminate_stuck_execution(request_id, future):
    # type: (str, futures_compat.Future) -> Dict[str, Any]
    """
    ...
    * ``result``: the future's result dict when resolved, else None.
    ...
    # raise CompatInterruptedError at the next cycle boundary even if L2 is
    ...
    except futures_compat.TimeoutError:
...
def execute_snippet(ctx, code, request_id, timeout_s):
    ...
    if has_running:
        ...
        future = submit_snippet(code, request_id)  # type: futures_compat.Future
        path = "callback"
    else:
