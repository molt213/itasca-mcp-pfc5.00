...
class FileBuffer:
    ...
    def __init__(self, log_path, buffer_size=8192):
        ...
        self._file = io.open(log_path, 'w', encoding='utf-8', buffering=buffer_size)
    ...
    def write(self, s):
        ...
        # Python 2.7 compat: io.open(encoding=...) expects unicode, but
        # print() in Python 2.7 produces str (bytes). Decode if needed.
...
class TeeBuffer:
    ...
    def write(self, s):
        if not s:
            return 0
        # Python 2.7 compat: ensure unicode for io.TextIOWrapper
        if isinstance(s, bytes):
            s = s.decode('utf-8', 'replace')
